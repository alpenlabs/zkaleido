...
impl ZkVmVerifier for Risc0Host {
    ...
    fn get_vk(&self) -> VerifyingKey {
        VerifyingKey::new(self.id.as_bytes().to_vec())
    }

    fn get_vk_commitment(&self) -> VerifyingKeyCommitment {
        VerifyingKeyCommitment::new(self.id.into())
    }
